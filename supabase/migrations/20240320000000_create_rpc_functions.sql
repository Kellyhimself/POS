...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO transactions (
            ...
            p_vat_total,
            NOW(),
            TRUE  -- Mark as synced since it's created directly in the database
        )
        ...
        LOOP
            ...
            INSERT INTO sale_items (
                ...
                COALESCE(v_product->>'saleMode', 'retail'),
                NOW()
            );
            ...
            UPDATE products
            SET 
                quantity = quantity - (v_product->>'quantity')::INTEGER,
                updated_at = NOW()
            WHERE 
                id = (v_product->>'id')::UUID
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Rollback the transaction on error
            RAISE EXCEPTION 'Error in sale creation: %', SQLERRM;
    END;
END;
$$;
...
GRANT EXECUTE ON FUNCTION create_sale(UUID, JSONB, TEXT, DECIMAL, DECIMAL) TO authenticated; 