...
is this system able to serve a wholesale business that has lets say two devices, one in front office (POS page) and another for back office(inventory and reports page), assume two cases:
1. both devices are connected to the internet, so both in both they have installed this app, an the @useGlobalProductSync.ts , @useGlobalSaleSync.ts  and @useGlobalProductCache.ts  and @useGlobalEtimsSync.ts  are running in the background. Then we have say two users, the front office one is busy making sales, leading to his local storage stock updates and since he is offline the hooks are busy updating the online storage. at the same time, in the back office another user, say the admin is busy inputting new stock and updating stock quantities, the hooks are also updating the online db with the inventory updates since he is online too. will these lead to wrong quantities and data confusion during the sync processes?  all this system is working perfect? If the system is faulty, which is the easiet option we have? scafold this app and create a new version of the app that only works online to support multiple devices for the same store, or modify the existing app to improve the sync processes? which is easier, personally i was thinking of having another version, but do your research, i know you know better ways, then advice which approach we should follow or is our system perfect?

2. Imagine a similar scenario, (front and back office) and both devices are offline?

3. this app has most of its core functionality relying on offline and syncing to the cloud. so it is tailored for offline first businesses, who can                                                                                                   just expect to operate with no internet. I think this is a limitation even if the app allows sync into cloud. The limitation is that, while it supports offline functionality and online sync, it wont allow multi user flexibility, say one store having launched the app on different screens. I was thinking a solution to this, should we build a different version of this that will only operate with the internet which will give users a flexibility to operate their stores with more than one device